...
test("keeps dataset metadata, schema and prompt records parseable", async () => {
  const files = [
    ...
    path.join(projectRoot, "prompt", "2026-08-24-stage-04-model-adapters-reconstructed.json"),
    path.join(projectRoot, "prompt", "2026-08-24-stage-05-yolo11n-training-reconstructed.json")
  ];
  const parsed = [];
